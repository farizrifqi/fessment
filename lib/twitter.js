...
export function FessmentTwitter() {
    ...
    return {
        ...
        async getDirectMessages(token) {
            let request = await fetch(API_URL + '/dm_events?dm_event.fields=sender_id&expansions=sender_id,attachments.media_keys&media.fields=media_key,preview_image_url,url,variants&user.fields=username',
                {
                    method: "GET",
                    headers: {
                        "Content-type": "application/json",
        ...
        async sendDirectMessages(token, id, message) {
            let request = await fetch(API_URL + '/dm_conversations/with/' + id + '/messages',
                {
                    method: "POST",
                    headers: {
                        "Content-type": "application/json",
